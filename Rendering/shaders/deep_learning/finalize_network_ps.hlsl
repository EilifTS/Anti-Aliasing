...
float3 manualBilinear(float2 uv)
{
	...
	float3 g00 = input_texture[int2(pos0.x, pos0.y)].rgb;
	float3 g01 = input_texture[int2(pos0.x, pos1.y)].rgb;
	float3 g10 = input_texture[int2(pos1.x, pos0.y)].rgb;
	float3 g11 = input_texture[int2(pos1.x, pos1.y)].rgb;
...
{ 
	...
	jau_rgbd.rgb = half3(input_texture.Sample(linear_clamp, hr_jitter_uv).rgb);
